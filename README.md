...
- Day 11 – Caching & Performance Optimization with Redis: installed Memurai (Windows-native Redis-compatible server), connected Flask to Redis via redis-py, and practiced core commands (SET/GET/DEL) with TTL expiration. Implemented the Cache-Aside pattern on GET /auth/me with a 5-minute TTL, and added cache invalidation on PUT/DELETE /auth/me so updates are never served stale. Verified caching and invalidation directly via Redis, and noted that response-time differences weren't dramatic on local SQLite since it's already fast for single-row lookups. Week 3 Day 1 complete.
- Day 12 – Asynchronous Messaging & Task Queues: implemented a Producer/Consumer pattern using RQ (Redis Queue) built on the existing Redis (Memurai) setup. Registration now enqueues a simulated welcome email job instead of blocking the request. Built a Windows-compatible worker (SimpleWorker) to process jobs independently, added automatic retries (max 3 attempts), and verified failed jobs land in RQ's Failed Job Registry (Dead-Letter Queue). Diagnosed and fixed a Windows-specific "localhost" resolution delay, and measured response times with vs. without the queue to demonstrate the performance benefit. Week 3 Day 2 complete.